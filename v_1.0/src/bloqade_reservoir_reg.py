...
#jl.include("src\\bloqade_call_modified.jl")
#jl.include("src\\bloqade_call_modified.jl")
#jl.include(")
...
class BloqadeReservoir:
    ...
    """
    def __init__(self, t_start: float, t_end: float, step: float, omega_max: float, nsites: int, d: float, lattice_type: str)->None:
    ...
    def __call__(self,x: np.array, MIN: np.float64, MAX: np.float64)->np.array:
        ...
        arguments:
        >>> x: np.array of shape (1, len_seq)

        returns: a batched np array of shape (1, res_Dim), 
            where values for each batch element as gathered as densities of bloade atoms.
        """
        x = np.squeeze(x, axis=1)
        return np.array(jl.bloqade_call_mod(x,self.t_start,self.t_end,self.step, self.omega_max, self.nsites, self.distance, MIN, MAX, self.lattice_type))